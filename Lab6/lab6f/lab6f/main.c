...
void SPI_init()
{
	...
	SPIF.CTRL = SPI_ENABLE_bm |				//enable, data order msb
				...
				SPI_PRESCALER_DIV4_gc;		//prescaler of 4
...
uint8_t gyroRead(uint8_t reg) {
	...
	PORTF.OUTCLR = 0x10;		//SSG, sensor select gyro
...
void accel_init() {
	...
	*/
	...
	accWrite((~0x80 & CTRL_REG4_A & 0x3F), 0x01); //enable accelerometer
	
	accWrite((~0x80 & CTRL_REG4_A & 0x3F), (CTRL_REG4_A_DR_EN |		//route DRDY to INT1_A and enable INT1 rising edge
											CTRL_REG4_A_IEA |		
											CTRL_REG4_A_INT1_EN |
											CTRL_REG4_A_STRT));			
			
	accWrite((~0x80 & CTRL_REG5_A & 0x3F), (CTRL_REG5_A_ODR3 |		//configure 1600Hz output rate and enable XYZ
											CTRL_REG5_A_ODR0 |
											CTRL_REG5_A_ZEN |
											CTRL_REG5_A_YEN |
											CTRL_REG5_A_XEN));		
...
void gyro_init() {
	...
	*/
	...
	gyroWrite((~0x80 & CTRL_REG1_G & 0x3F), 0x01 );
	
	gyroWrite((~0x80 & CTRL_REG1_G & 0x3F), (CTRL_REG1_G_DR1 |		//highest 100Hz, enable XYZ, normal mode
											CTRL_REG1_G_DR0 |
											CTRL_REG1_G_PD	|
											CTRL_REG1_G_ZEN |
											CTRL_REG1_G_YEN |
	...
	gyroWrite((~0x80 & CTRL_REG3_G & 0x3F), CTRL_REG3_G_I2_DRDY);	//set I2_DRDY
	
	gyroWrite((~0x80 & CTRL_REG4_G & 0x3F), (CTRL_REG4_G_FS1 |
